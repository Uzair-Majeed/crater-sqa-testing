describe('Customer Management - Black Box Testing', () => {
    ...
    describe('1. Customer Creation Tests', () => {
        ...
        it('TC-001: Should successfully create a new customer with all required fields', () => {
            ...
            // Use eq(0) because there are multiple inputs with name="name" (Display Name and Prefix)
        ...
        it('TC-003: Should show validation error when name is empty', () => {
            ...
            // Verify validation error - Actual text from en.json is "Field is required"
        ...
        it('TC-004: Should show validation error for invalid email format', () => {
            ...
            // Verify validation error - Actual text from en.json is "Incorrect Email."
    ...
    describe('5. Advanced Creation Scenarios', () => {
        ...
        it('TC-007: Should create a customer with ALL fields filled (Basic, Billing, Shipping)', () => {
            ...
            // --- Basic Info ---
            cy.get('input[name="name"]').eq(0).type(uniqueName); // Display Name
            // Primary Contact Name (no name attribute, use parent label)
            ...
            cy.get('input[name="phone"]').eq(0).type('+15550000000'); // Basic Phone
            cy.get('input[type="url"]').type('https://craterapp.com'); // Website
            cy.get('input[name="name"]').eq(1).type('Mr.'); // Prefix (2nd input with name="name")

            // Currency (Multiselect - assuming default is USD, but let's try to select if possible or leave default)
            // Leaving default to avoid complexity with custom dropdowns for now, unless required.

            // --- Billing Address ---
            cy.get('input[name="address_name"]').eq(0).type(`Billing Name ${timestamp}`);
            // Country (Multiselect) - skip for now or try to type if it's a searchable input
            // cy.get('.multiselect').eq(1).click().type('United States{enter}'); 
            ...
            cy.get('input[name="phone"]').eq(1).type('+15551111111'); // Billing Phone
            cy.get('input[name="zip"]').eq(0).type('90001'); // Billing Zip
            ...
            cy.get('input[name="phone"]').eq(2).type('+15552222222'); // Shipping Phone
            cy.get('input[name="zip"]').eq(1).type('89109'); // Shipping Zip
