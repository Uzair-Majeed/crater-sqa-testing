...
if (!function_exists('base_path')) {
    ...
    {
        // In this test context, base_path returns cwd for empty path, else absolute path.
        if ($path === '') {
            return getcwd();
        }
        // Accept absolute paths as-is, otherwise append to cwd.
...
test('check method sets isSet to true for all folders when permissions are met', function () {
    // Instead of mocking private method, create real temp folders/files with required permissions
    $checker = new FilePermissionChecker();

    // Setup temp folders/files
    $tempItems = [];
    $folders = [];

    // Folder 1: 0755
    $dir1 = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir1_');
    ...
    // File 2: 0644
    $file2 = tempnam(sys_get_temp_dir(), 'fpct_file2_');
    ...
    // Folder 3: 0777
    $dir3 = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir3_');
    ...
    try {
        ...
        expect($results['permissions'][0])->toMatchArray(['folder' => $dir1, 'permission' => '0755', 'isSet' => true]);
        expect($results['permissions'][1])->toMatchArray(['folder' => $file2, 'permission' => '0644', 'isSet' => true]);
        expect($results['permissions'][2])->toMatchArray(['folder' => $dir3, 'permission' => '0777', 'isSet' => true]);
    } finally {
        // Clean up
...
test('check method sets isSet to false and errors to true when some permissions are not met', function () {
    ...
    try {
        ...
        expect($results['permissions'][0])->toMatchArray(['folder' => $dir1, 'permission' => '0755', 'isSet' => false]);
        expect($results['permissions'][1])->toMatchArray(['folder' => $file2, 'permission' => '0644', 'isSet' => true]);
        expect($results['permissions'][2])->toMatchArray(['folder' => $dir3, 'permission' => '0777', 'isSet' => false]);
    } finally {
        foreach ($tempItems as $item) {
            if (is_dir($item)) {
                rmdir($item);
...
test('check method handles a single folder with sufficient permission', function () {
    ...
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir_singlepass_');
    mkdir($dir, 0777, true);
    chmod($dir, 0777);
    ...
    try {
        ...
        expect($results['errors'])->toBeNull()
            ...
            ->and($results['permissions'][0])->toMatchArray(['folder' => $dir, 'permission' => '0755', 'isSet' => true]);
    } finally {
        rmdir($dir);
    }
});
...
test('check method handles a single folder with insufficient permission', function () {
    ...
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir_singlefail_');
    mkdir($dir, 0700, true);
    chmod($dir, 0700);
    ...
    try {
        ...
        expect($results['errors'])->toBeTrue()
            ->and($results['permissions'])->toHaveCount(1)
            ->and($results['permissions'][0])->toMatchArray(['folder' => $dir, 'permission' => '0755', 'isSet' => false]);
    } finally {
        rmdir($dir);
    }
});
...
test('check method correctly handles zero required permission', function () {
    ...
    // A real file/folder -- permission is irrelevant due to required '0000'
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir_any_');
    mkdir($dir, 0700, true);
    chmod($dir, 0700);
    ...
    try {
        ...
        expect($results['errors'])->toBeNull()
            ->and($results['permissions'])->toHaveCount(1)
            ->and($results['permissions'][0])->toMatchArray(['folder' => $dir, 'permission' => '0000', 'isSet' => true]);
    } finally {
        rmdir($dir);
    }
});
...
test('check method handles high required permission that is not met', function () {
    ...
    $dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('fpct_dir_secure_');
    mkdir($dir, 0755, true);
    chmod($dir, 0755);

    try {
        $folders = [$dir => '0777']; // Required 0777, but folder is 0755
        $results = $checker->check($folders);

        expect($results['errors'])->toBeTrue()
            ->and($results['permissions'])->toHaveCount(1)
            ->and($results['permissions'][0])->toMatchArray(['folder' => $dir, 'permission' => '0777', 'isSet' => false]);
    } finally {
        rmdir($dir);
    }
});
