...
test('rules for POST method includes unique name without ignore clause and correct company_id', function () {
    ...
    // FIX: The original test checked `instanceof Rule`, which might fail if the `PaymentMethodRequest`
    // (even as a partial mock) returns string rules directly, or if the `Rule` object isn't resolved
    // correctly in a mocked context for `instanceof` check. By removing `instanceof Rule` and
    // relying on `str_contains((string) $rule, ...)` alone, we can correctly identify the unique rule,
    // as `Rule` objects implement `__toString()`.
    $uniqueRule = collect($rules['name'])->first(fn ($rule) => str_contains((string) $rule, 'unique:payment_methods'));
...
test('rules for POST method with null company header includes unique name with empty company_id', function () {
    ...
    // FIX: Removed `instanceof Rule` for reasons mentioned above.
    $uniqueRule = collect($rules['name'])->first(fn ($rule) => str_contains((string) $rule, 'unique:payment_methods'));
    expect($uniqueRule)->not->toBeNull();
    // Expected format: unique:table,column,ignoreId,idColumn,whereColumn,whereValue
    // For null company ID, the value part for company_id will be empty, as per test expectation.
