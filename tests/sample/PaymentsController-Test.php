...
test('index method returns a collection of payments with default limit and no filters', function () {
    ...
    // Fix: Instead of direct property assignment which calls setAttribute, mock the getAttribute method
    $mockPayment1 = Mockery::mock(Payment::class);
    $mockPayment1->shouldReceive('getAttribute')->with('id')->andReturn(1);
    $mockPayment2 = Mockery::mock(Payment::class);
    $mockPayment2->shouldReceive('getAttribute')->with('id')->andReturn(2);
...
test('show method returns a single payment resource when payment is found', function () {
    ...
    // Fix: Instead of direct property assignment which calls setAttribute, mock the getAttribute method
    $mockPayment->shouldReceive('getAttribute')->with('id')->andReturn($paymentId);
