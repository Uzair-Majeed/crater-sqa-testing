...
test('it returns all and active used currencies when provider ID is null and data exists', function () {
    ...
    // Directly set properties for easier access simulation, if controller accesses them this way.
    // If the controller strictly uses $request->get(), then the shouldReceive('get') is sufficient.
    $request->provider_id = null; 
    $request->shouldReceive('get')->with('provider_id')->andReturn(null)->once();
    ...
    // Fix: Replace Pest's `mock()` helper with Mockery alias mock for static calls to ExchangeRateProvider.
    // This sets up an alias mock for static methods on the ExchangeRateProvider class.
    $exchangeRateProviderMock = Mockery::mock('alias:' . ExchangeRateProvider::class);
    $exchangeRateProviderMock->shouldReceive('where')
        ->with('active', true)
        ->andReturn($activeQueryBuilderMock) // Returns the configured Builder mock for the active chain
        ->once();
    $exchangeRateProviderMock->shouldReceive('whereCompany')
...
test('it filters active used currencies when provider ID is present', function () {
    ...
    $request->shouldReceive('get')->with('provider_id')->andReturn($providerId)->once();
    ...
    // Fix: Replace Pest's `mock()` helper with Mockery alias mock
    $exchangeRateProviderMock = Mockery::mock('alias:' . ExchangeRateProvider::class);
    $exchangeRateProviderMock->shouldReceive('where')
        ->with('active', true)
