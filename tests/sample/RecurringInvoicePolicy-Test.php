...
test('view allows access when user has permission AND belongs to the recurring invoice company', function () {
    ...
    // Policies typically read properties, they don't set them.
    // To simulate `$recurringInvoice->company_id` returning a value, mock `getAttribute`.
    $recurringInvoice->shouldReceive('getAttribute')
                     ->with('company_id')
                     ->andReturn($companyId)
                     ->atLeast()->once(); // Policy will access this
...
test('view denies access when user does NOT have permission to view recurring invoice (short-circuit)', function () {
    ...
    // Simulate property access (even if short-circuited, it's good practice for the mock to be consistent)
    $recurringInvoice->shouldReceive('getAttribute')
                     ->with('company_id')
                     ->andReturn($companyId)
                     ->zeroOrMoreTimes(); // Might not be called if `can` short-circuits
