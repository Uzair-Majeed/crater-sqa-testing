...
test('rules returns correct rules for POST request (create)', function () {
    ...
    // FIX: Instead of trying to set a non-existent property via reflection,
    // mock the 'input' method, as 'type' would typically come from the request input.
    $request->shouldReceive('input')->with('type')->andReturn($type);
