...
function getCustomFieldValueKey($type)
{
    return match ($type) {
        'TEXT' => 'string_answer',
...
function clean_slug($model_type, $name)
{
    return 'clean-slug-' . $model_type . '-' . $name;
}
...
test('setTimeAnswerAttribute does nothing if value is null or empty', function () {
    ...
    expect($model->getAttributes()['time_answer'])->toBe('initial_value');
...
test('scopeWhereCompany applies correct company_id filter', function () {
    ...
    // Simulate request()->header('company') returning $companyId in the test context
    RequestFacade::swap(new class($companyId) {
        public $companyId;
        public function __construct($companyId) { $this->companyId = $companyId; }
        public function header($header) { return $header === 'company' ? $this->companyId : null; }
    });
...
test('scopeWhereSearch applies correct label and name search filters', function () {
    ...
    $mockQuery->shouldReceive('where')
        ->once()
        ->with(Mockery::on(function ($callback) use ($search, $mockQuery) {
...
test('scopeApplyFilters applies type filter when present', function () {
    ...
    // Setup return value
    $mockQuery->shouldReceive('getQuery')->andReturn($mockQuery);
