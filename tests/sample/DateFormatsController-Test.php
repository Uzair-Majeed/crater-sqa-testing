...
test('it returns a list of date formats from DateFormatter::get_list', function () {
    ...
    // Convert JsonResponse to TestResponse to be able to use Pest/Laravel assertions.
    $testResponse = TestResponse::fromBaseResponse($response);

    // Assert
    $testResponse->assertStatus(200); // Asserts HTTP 200 OK status
...
test('it returns an empty list when DateFormatter::get_list returns an empty array', function () {
    ...
    // Assert
    $testResponse->assertStatus(200); // Still a successful response, just with empty data.
    $testResponse->assertJson([
