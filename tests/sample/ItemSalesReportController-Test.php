...
// Define common expected setting keys for CompanySetting whereIn calls
const COMPANY_SETTING_COLOR_KEYS = [
...
test('it streams the item sales report PDF by default', function () {
    ...
    // Remove direct property assignment, rely on getAttribute for mocks
    ...
    // Make CompanySetting::getSetting calls more explicit
    ...
    // Corrected mocking for CompanySetting::whereIn()->whereCompany()->get() chain
    CompanySetting::shouldReceive('whereIn')
        ->once()
        ->with(Mockery::subset(COMPANY_SETTING_COLOR_KEYS))
        ->andReturn($mockCompanySettingQuery);
    // Removed redundant ->shouldReceive('whereIn') on the query mock itself
...
test('it downloads the item sales report PDF when download flag is present', function () {
    ...
    $dateFormat = 'd/m/Y';
    $currencyId = 1;
    $currencyCode = 'USD'; // Define currency code for consistency

    $mockCompany = Mockery::mock(Company::class);
    // Remove direct property assignment
    // $mockCompany->id = $companyId;
    $mockCompany->shouldReceive('getAttribute')->with('id')->andReturn($companyId);
    $mockCompany->shouldReceive('getAttribute')->with('unique_hash')->andReturn($hash);
    Company::shouldReceive('where->first')
        ->once()
        ->with('unique_hash', $hash) // Add specific 'with' clause
        ->andReturn($mockCompany);
    ...
    CompanySetting::shouldReceive('whereIn')
        ->once()
        ->with(Mockery::subset(COMPANY_SETTING_COLOR_KEYS)) // Add specific 'with' clause
        ->andReturn($mockCompanySettingQuery);
    // Removed redundant ->shouldReceive('whereIn') on the query mock itself
    $mockCompanySettingQuery->shouldReceive('whereCompany')
        ->once()
    ...
    $mockCompanySettingQuery->shouldReceive('get')
        ->once()
        ->andReturn(collect()); // Assuming empty collection for this test
    ...
    $mockCurrency = Mockery::mock(Currency::class);
    // Remove direct property assignment and add getAttribute expectation
    ...
    App::shouldReceive('setLocale')->once()->with($locale); // Add specific 'with' clause
...
test('it includes color settings in shared view data', function () {
    ...
    $mockCompanySettingQuery = Mockery::mock();
    // This part of the test already correctly mocks the CompanySetting chain
    CompanySetting::shouldReceive('whereIn')
        ->once()
        ->with(Mockery::subset(COMPANY_SETTING_COLOR_KEYS))
        ->andReturn($mockCompanySettingQuery);
