...
beforeEach(function () {
    ...
    test('view returns true if user can view specific invoice and has company access', function () {
        ...
        // FIX: Added makePartial() to allow setting properties like company_id directly.
        // Direct property assignment `$invoice->company_id = ...` on a pure Mockery mock
        // for an Eloquent Model calls `setAttribute`, which requires an expectation.
        // `makePartial()` allows the real method to be called if no specific expectation is set.
    ...
    test('update returns true if user can edit invoice, has company access, and invoice is editable', function () {
        $invoiceCompanyId = 1;
        // FIX: Added makePartial() to allow setting properties
        $invoice = Mockery::mock(Invoice::class)->makePartial();
        $invoice->company_id = $invoiceCompanyId;
