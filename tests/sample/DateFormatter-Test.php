...
beforeEach(function () {
    // Set up Carbon mocking before each test using alias only once.
    if (!class_exists('Mockery_0_Carbon_Carbon')) {
        Mockery::mock('alias:' . Carbon::class);
    }
});

test('get_list returns a list of date formats with formatted dates', function () {
    // 1. Arrange: Set up method mocks on the aliased Carbon facade
    $carbonInstanceMock = Mockery::mock();
    $fixedDate = '2023-01-15 10:30:00';

    Carbon::shouldReceive('now')
        ->andReturn($carbonInstanceMock);

    $carbonInstanceMock->shouldReceive('format')->with('Y M d')->andReturn('2023 Jan 15');
    $carbonInstanceMock->shouldReceive('format')->with('d M Y')->andReturn('15 Jan 2023');
    $carbonInstanceMock->shouldReceive('format')->with('d/m/Y')->andReturn('15/01/2023');
    $carbonInstanceMock->shouldReceive('format')->with('d.m.Y')->andReturn('15.01.2023');
    $carbonInstanceMock->shouldReceive('format')->with('d-m-Y')->andReturn('15-01-2023');
    $carbonInstanceMock->shouldReceive('format')->with('m/d/Y')->andReturn('01/15/2023');
    $carbonInstanceMock->shouldReceive('format')->with('Y/m/d')->andReturn('2023/01/15');
    $carbonInstanceMock->shouldReceive('format')->with('Y-m-d')->andReturn('2023-01-15');
...
test('get_list returns an empty array when no formats are defined', function () {
    ...
    $originalFormats = $property->getValue();

    $property->setValue(null, []);

    Carbon::shouldNotReceive('now');
