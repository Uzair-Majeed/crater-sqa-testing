...
test('rules method returns correct validation rules for POST request (user creation)', function () {
    ...
    // When testing Unique rules, it's best to assert against their string representation
    // as their internal properties are protected.
    expect((string) $uniqueRule)->toEqual('unique:users,email');
...
test('rules method returns correct validation rules for PUT request (user update)', function () {
    ...
    $uniqueRule = extractRuleOfType($rules['email'], Unique::class);
    expect($uniqueRule)->toBeInstanceOf(Unique::class);
    // Assert against the string representation for the unique rule with an ignored ID
    expect((string) $uniqueRule)->toEqual("unique:users,email,{$userIdToIgnore},id");
