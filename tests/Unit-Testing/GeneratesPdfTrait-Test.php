...
beforeEach(function () {
    ...
    // Global mocks for static facades
    Mockery::mock('alias:' . CompanySetting::class)->makePartial();
    Mockery::mock('alias:' . App::class)->makePartial();
    Mockery::mock('alias:' . Storage::class)->makePartial();
    Mockery::mock('alias:' . FileDisk::class)->makePartial();
    Mockery::mock('alias:' . Carbon::class)->makePartial();
    Mockery::mock('alias:' . Response::class)->makePartial(); // For response() helper
...
test('getGeneratedPDFOrStream falls back to streaming when generated PDF does not exist', function () {
    ...
    // Mock CompanySetting::getSetting for language
...
test('getGeneratedPDF returns false when FileDisk not found for media', function () {
    ...
    // Mock FileDisk::find to return null
...
test('getGeneratedPDF returns temporary URL for remote driver', function () {
    ...
    $mockMedia->file_name = $expectedFileName;
    // Mock Carbon::now() and its addMinutes method for the temporary URL generation
    Carbon::shouldReceive('now')->once()->andReturn(Mockery::mock(\DateTime::class));
    Carbon::shouldReceive('now->addMinutes')->once()->andReturn(Mockery::mock(\DateTime::class));
...
test('generatePDF throws exception if no default FileDisk is found', function () {
    ...
    // Mock FileDisk::whereSetAsDefault to return null
    FileDisk::shouldReceive('whereSetAsDefault')
        ->once()
        ->andReturnSelf();
    FileDisk::shouldReceive('first')
...
test('getFormattedString removes unused placeholders and cleans empty html tags', function () {
    ...
    $expected = "<div><br />Some text<br /></div><br />
               Used Value and .
...
test('getFormattedString handles format string with only placeholders and HTML', function () {
    ...
    $expected = "<h1>Report</h1><br />Generated on 2023-01-01.</br><br />
               <br />
               </br>";
