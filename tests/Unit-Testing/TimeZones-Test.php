...
test('get_list returns a complete and correct array of timezones', function () {
    $expectedTimezones = [
    ...
    $timezones = TimeZones::get_list();

    // Ensure the returned value is an array
    expect($timezones)->toBeArray();

    // Ensure the returned array is not empty
    expect($timezones)->not->toBeEmpty();

    // Assert that the count of timezones matches the expected count
    expect(count($timezones))->toBe(count($expectedTimezones));

    // Assert that the entire returned array exactly matches the predefined list.
    // This covers all entries, their keys, values, and the overall structure and order.
    expect($timezones)->toEqual($expectedTimezones);

    // Specific checks for robustness and white-box coverage of known entries:
    // Check the very first entry
    expect($timezones[0])->toEqual(['value' => 'Pacific/Midway', 'key' => '(UTC-11:00) Midway']);

    // Check a middle entry, e.g., the UTC reference
    expect($timezones)->toContain(['value' => 'UTC', 'key' => '(UTC+00:00) UTC']);
