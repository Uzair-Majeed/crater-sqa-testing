...
beforeEach(function () {
    ...
    // Mock Setting facade
    // Using alias makes Mockery intercept static calls to Setting::class
    $this->settingMock = Mockery::mock('alias:'.Setting::class);
    $this->settingMock->shouldReceive('getSetting')
        ->with('version')
        ->andReturn('1.0.0')
        ->byDefault();

    // Mock the config helper value via its facade
    ...
    // Mock the url helper value via its facade
    $this->urlMock = Mockery::mock('alias:Illuminate\Support\Facades\URL');
    $this->urlMock->shouldReceive('to')
        ->with('/')
...
test('it constructs Guzzle client with correct base_uri and verify option', function () {
    ...
    Mockery::mock('alias:Illuminate\Support\Facades\Config')
        ->shouldReceive('get')->with('crater.base_url')->andReturn('http://test.api.com')->byDefault();
    Mockery::mock('alias:Illuminate\Support\Facades\URL')
        ->shouldReceive('to')->with('/')->andReturn('http://test.app.com')->byDefault();
