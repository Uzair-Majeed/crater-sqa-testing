...
test('build method creates email log and sets properties without attachment', function () use ($testData) {
    ...
    $emailLogMock = Mockery::mock(EmailLog::class);
    $emailLogMock->shouldReceive('save')->once();
    $emailLogMock->id = 123;
    $emailLogMock->token = null; // Will be set by Hashids facade
    ...
    // The original code passes ['data', $this->data] - test this exact structure.
    $mail->shouldReceive('markdown')
        ->with('emails.send.invoice', ['data', $testData])
        ->andReturn($mail)
        ->once();
...
test('build method creates email log and sets properties with attachment', function () use ($testData) {
    ...
    // The original code passes ['data', $this->data] - test this exact structure.
    $mail->shouldReceive('markdown')
        ->with('emails.send.invoice', ['data', $testDataWithAttachment])
        ->andReturn($mail)
        ->once();
