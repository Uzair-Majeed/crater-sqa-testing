...
test('rules method returns correct validation rules for POST request', function () {
    ...
    expect($uniqueRule->table)->toBe('units');
    expect($uniqueRule->column)->toBe('name'); // By default, column is the attribute name if not specified

    // Use reflection to access protected 'where' property of the Unique rule
    $reflectionProperty = new \ReflectionProperty($uniqueRule, 'where');
    $reflectionProperty->setAccessible(true);
    $whereClauses = $reflectionProperty->getValue($uniqueRule);

    expect($whereClauses)->toBeArray()
        ->and($whereClauses)->toHaveKey('company_id', $companyId);

    // Ensure 'ignore' is not set for POST requests
    $reflectionProperty = new \ReflectionProperty($uniqueRule, 'ignore');
    $reflectionProperty->setAccessible(true);
    $ignoreValue = $reflectionProperty->getValue($uniqueRule);
...
test('rules method returns correct validation rules for PUT request', function () {
    ...
    expect($uniqueRule)->not->toBeNull('A Unique rule should be present for the name field.');
    expect($uniqueRule->table)->toBe('units');
    expect($uniqueRule->column)->toBe('name');
    ...
    $reflectionProperty = new \ReflectionProperty($uniqueRule, 'idColumn');
    $reflectionProperty->setAccessible(true);
    $idColumnValue = $reflectionProperty->getValue($uniqueRule);
...
test('rules method handles null company header for POST request gracefully', function () {
    ...
    expect($whereClauses)->toBeArray()
        ->and($whereClauses)->toHaveKey('company_id', $companyId); // Assert it's passed as null
});
