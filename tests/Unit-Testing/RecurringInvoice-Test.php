...
beforeEach(function () {
    ...
    m::mock('alias:' . Invoice::class); // For static calls
    m::mock('alias:' . Customer::class); // For static find in createInvoice
    m::mock('alias:' . Company::class); // For static find in createInvoice
...
test('createFromRequest creates recurring invoice and handles dependencies', function () {
    ...
    // Mock addCustomFields trait method
...
test('createItems creates invoice items and their taxes', function () {
    ...
    $item2Mock->shouldReceive('taxes')->andReturn(m::mock(HasMany::class)->shouldNotReceive('create')->getMock()); // No tax creation as amount is null
    ...
    $itemRelationMock->shouldReceive('create')->once()->with(['name' => 'Item 1', 'taxes' => [['name' => 'Tax1', 'amount' => 10.0]], 'company_id' => 1])->andReturn($item1Mock);
    $itemRelationMock->shouldReceive('create')->once()->with(['name' => 'Item 2', 'taxes' => [['name' => 'Tax2', 'amount' => 'NULL']], 'company_id' => 1])->andReturn($item2Mock);
    $itemRelationMock->shouldReceive('create')->once()->with(['name' => 'Item 3', 'company_id' => 1])->andReturn(m::mock(InvoiceItem::class)->shouldNotReceive('taxes')->getMock());
...
test('generateInvoice creates invoice and updates date when limit_by is DATE and not expired', function () {
    ...
    Carbon::shouldReceive('today')->andReturn(Carbon::create(2023, 1, 15)->format('Y-m-d')); // Added format
    $recurringInvoice = m::mock(RecurringInvoice::class)->makePartial();
...
test('getNextInvoiceDate calculates and formats the next run date', function () {
    ...
    $formattedCarbon = m::mock(Carbon::class);
    $formattedCarbon->shouldReceive('format')->with('Y-m-d H:i:s')->andReturn('2023-02-01 00:00:00');
    $nextRunDate->shouldReceive('format')->andReturn($formattedCarbon->format('Y-m-d H:i:s'));
...
test('Carbon::parse can be mocked globally', function () {
    ...
    CompanySetting::shouldReceive('getSetting')->andReturn('Y-m-d');
...
test('createInvoice uses default due date days if company setting is null or "null"', function () {
    ...
    $newInvoiceMock->shouldReceive('save')->once();
    $newInvoiceMock->shouldReceive('addCustomFields')->once();
    $newInvoiceMock->shouldReceive('send')->once();
