...
// Assuming PDF is a globally aliased class or Facade.
// If it's a specific class, add its `use` statement, e.g., `use Barryvdh\DomPDF\Facade\Pdf as PDF;`
// For the purpose of this fix, we assume it's globally resolvable via Mockery.
...
beforeEach(function () {
    ...
    // Mock PDF Facade
    // Using explicit \PDF to ensure global namespace resolution if there are local PDF classes.
    \PDF::shouldReceive('loadView')->andReturnSelf()->zeroOrMoreTimes();
    \PDF::shouldReceive('stream')->andReturn('PDF Stream Content')->zeroOrMoreTimes();
    \PDF::shouldReceive('download')->andReturn('PDF Download Content')->zeroOrMoreTimes();

    // Mock App Facade
    App::shouldReceive('setLocale')->zeroOrMoreTimes();

    // Mock View Facade for view()->share() and view()->make()
    View::shouldReceive('share')->zeroOrMoreTimes();
    View::shouldReceive('make')->zeroOrMoreTimes(); // Added for preview test where `view()->make` is called

    // Mock Carbon for date formatting
    // To mock static methods like Carbon::createFromFormat(), we need an alias mock.
    // This will intercept static calls to Carbon.
    $carbonAliasMock = Mockery::mock('alias:'.Carbon::class);

    // This is a default Carbon *instance* mock that will be returned by Carbon::createFromFormat()
    // unless a specific test explicitly overrides this expectation.
    $mockCarbonInstance = Mockery::mock(Carbon::class);
    $mockCarbonInstance->shouldReceive('format')
        ->andReturnUsing(fn ($format) => 'Formatted Date ' . $format)
        ->zeroOrMoreTimes(); // Allow this instance to be formatted multiple times

    // Set the default expectation for Carbon::createFromFormat() to return our mock instance.
    // `zeroOrMoreTimes()` allows specific tests to add `once()` expectations for matching arguments later.
    $carbonAliasMock->shouldReceive('createFromFormat')
...
test('it generates profit and loss report stream successfully with valid data', function () {
    ...
    // Fix: Mock Company::where('unique_hash', $hash)->first() correctly
    $mockCompanyBuilder = Mockery::mock(\Illuminate\Database\Eloquent\Builder::class);
    $mockCompanyBuilder->shouldReceive('first')
        ->once()
        ->andReturn($mockCompany);
    Company::shouldReceive('where')
        ->once()
        ->with('unique_hash', $hash)
        ->andReturn($mockCompanyBuilder);
    ...
    CompanySetting::shouldReceive('whereIn')
        ->once()
        ->with(['primary_text_color', 'footer_bg_color']) // Explicitly define with() for whereIn if controller expects specific options
...
test('it handles no expenses correctly, resulting in zero totalExpense', function () {
    ...
    // Fix: Mock Company::where()->first() correctly
    $mockCompanyBuilder = Mockery::mock(\Illuminate\Database\Eloquent\Builder::class);
    $mockCompanyBuilder->shouldReceive('first')->andReturn($mockCompany);
    Company::shouldReceive('where')->andReturn($mockCompanyBuilder);
...
test('it throws authorization exception if company not found', function () {
    ...
    // Fix: Mock Company::where('unique_hash', $hash)->first() to return null correctly
    $mockCompanyBuilder = Mockery::mock(\Illuminate\Database\Eloquent\Builder::class);
    $mockCompanyBuilder->shouldReceive('first')
        ->once()
        ->andReturn(null); // Explicitly return null
    Company::shouldReceive('where')
        ->once()
        ->with('unique_hash', $hash)
        ->andReturn($mockCompanyBuilder);
...
test('it handles different company date format settings', function () {
    ...
    // Fix: Interact with the Carbon alias mock for specific createFromFormat expectations
    $carbonAliasMock = Mockery::mock('alias:'.Carbon::class);

    // Mock Carbon specifically for this case, for the instances returned by createFromFormat
    $mockCarbonFrom = Mockery::mock(Carbon::class);
    $mockCarbonFrom->shouldReceive('format')->with('m/d/Y')->andReturn('01/01/2023')->once();
    $mockCarbonTo = Mockery::mock(Carbon::class);
    $mockCarbonTo->shouldReceive('format')->with('m/d/Y')->andReturn('01/31/2023')->once();

    // Override the default createFromFormat expectations for these specific calls
    $carbonAliasMock->shouldReceive('createFromFormat')
        ->once()
