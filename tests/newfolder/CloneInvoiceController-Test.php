...
/**
 * Helper to stub Invoice::create for global usage in a test,
 * solves "Could not load mock ... class already exists" issue.
 */
function stubStaticInvoiceCreate($return, $withSubset)
{
    $mockStatic = Mockery::mock('overload:' . Invoice::class);
    $mockStatic->shouldReceive('create')->once()
        ->with(Mockery::subset($withSubset))
        ->andReturn($return);
}
...
test('it clones an invoice successfully with automatic due date and all relations', function () {
    ...
    $originalInvoice = Mockery::mock('Crater\Models\Invoice[fields,load]', [$invoiceId])->makePartial();
    ...
    $item2 = [
        ...
        'taxes' => [
            ...
            ['id' => 103, 'name' => 'Tax C Zero', 'amount' => 0, 'pivot' => ['taxable_id' => 2, 'taxable_type' => 'Crater\Models\InvoiceItem', 'tax_id' => 103]],
        ],
    ];
    ...
    // If controller expects SerialNumberFormatter via service container, you may need to bind it as well, here we just mock it directly.
    ...
    $companySettingMock->shouldReceive('getSetting')->with('invoice_set_due_date_automatically', $companyId)
        ->andReturn('YES');
    $companySettingMock->shouldReceive('getSetting')->with('invoice_due_date_days', $companyId)
        ->andReturn($dueDateDays);

    // Mock New Invoice created by the controller
    $newInvoice = Mockery::mock('Crater\Models\Invoice[save,addCustomFields,items,taxes]', [$newInvoiceId])->makePartial();
    ...
    $mockItem1Obj = Mockery::mock(stdClass::class);
    $mockItem1TaxesRel = Mockery::mock(HasMany::class);
    $mockItem1TaxesRel->shouldReceive('create')
        ->once()
        ->with(Mockery::subset(['id' => 101, 'name' => 'Tax A', 'amount' => 2.5, 'company_id' => $companyId]));
    $mockItem1Obj->shouldReceive('taxes')->andReturn($mockItem1TaxesRel);
    ...
    $mockNewInvoiceItemsRelation->shouldReceive('create')
        ...
        )->andReturn($mockItem1Obj);

    $mockItem2Obj = Mockery::mock(stdClass::class);
    $mockItem2TaxesRel = Mockery::mock(HasMany::class);
    $mockItem2TaxesRel->shouldReceive('create')
        ->once()
        ->with(Mockery::subset(['id' => 102, 'name' => 'Tax B', 'amount' => 2.5, 'company_id' => $companyId]));
    $mockItem2TaxesRel->shouldNotReceive('create')
        ->with(Mockery::subset(['id' => 103])); // zero tax, not created
    $mockItem2Obj->shouldReceive('taxes')->andReturn($mockItem2TaxesRel);
    ...
    // Static Invoice::create via "overload:" workaround
    stubStaticInvoiceCreate($newInvoice, [
...
test('it clones an invoice successfully without automatic due date, items, top-level taxes, or custom fields', function () {
    ...
    $companySettingMock = Mockery::mock('alias:' . CompanySetting::class);
    $companySettingMock->shouldReceive('getSetting')->with('invoice_set_due_date_automatically', $companyId)->andReturn('NO');

    // Mock New Invoice created by the controller
    $newInvoice = Mockery::mock('Crater\Models\Invoice[save,items,taxes]', [$newInvoiceId])->makePartial();
    $newInvoice->id = $newInvoiceId;
    $newInvoice->unique_hash = null;
    $newInvoice->shouldReceive('save')->once();
    $newInvoice->shouldNotReceive('addCustomFields');
    $mockNewInvoiceItemsRel = Mockery::mock(HasMany::class);
    $mockNewInvoiceItemsRel->shouldNotReceive('create');
    $newInvoice->shouldReceive('items')->andReturn($mockNewInvoiceItemsRel);
    $mockNewInvoiceTaxesRel = Mockery::mock(HasMany::class);
    $mockNewInvoiceTaxesRel->shouldNotReceive('create');
    $newInvoice->shouldReceive('taxes')->andReturn($mockNewInvoiceTaxesRel);
    ...
    // Mock InvoiceResource constructor
    Mockery::mock('overload:' . InvoiceResource::class)->shouldReceive('__construct')->with($newInvoice)->once();
...
test('it clones an invoice with items having empty taxes array', function () {
    ...
    $originalInvoice->items = collect([(object) $item1]);
    $originalInvoice->taxes = collect([]);
    $mockFieldsRelation = Mockery::mock(HasMany::class);
    $mockFieldsRelation->shouldReceive('exists')->andReturn(false);
    $originalInvoice->shouldReceive('fields')->andReturn($mockFieldsRelation);
    ...
    $companySettingMock = Mockery::mock('alias:' . CompanySetting::class);
    $companySettingMock->shouldReceive('getSetting')->with('invoice_set_due_date_automatically', $companyId)->andReturn('YES');
    $companySettingMock->shouldReceive('getSetting')->with('invoice_due_date_days', $companyId)->andReturn($dueDateDays);
    ...
    $mockItem1Obj = Mockery::mock(stdClass::class);
    $mockItem1TaxesRel = Mockery::mock(HasMany::class);
    $mockItem1TaxesRel->shouldNotReceive('create');
    $mockItem1Obj->shouldReceive('taxes')->andReturn($mockItem1TaxesRel);

    $mockNewInvoiceItemsRelation->shouldReceive('create')
        ->with(Mockery::subset(array_merge($item1, [
            'company_id' => $companyId,
            'name' => 'Item A',
            'exchange_rate' => $exchangeRate,
            'base_price' => $item1['price'] * $exchangeRate,
