...
test('invoke authorizes module management', function () {
    ...
    // FIX: Instead of creating a partial mock and then spying on it,
    // create a partial mock that is also a spy directly. This avoids potential
    // nested mock class definition issues that can lead to redeclaration errors.
    $controller = Mockery::spy(ModuleController::class)->makePartial();
    // Assuming 'authorize' is a protected method, we need to allow mocking it
    // for the spy to correctly observe calls to it.
    ...
    // Mock the static 'collection' method of ModuleResource, which might be called
    // if the controller processes a list of modules, even if it's an empty list.
    Mockery::mock('overload:'.ModuleResource::class)
        ->shouldReceive('collection')
        ->andReturn(Mockery::mock(Collection::class));

    // Call the method on the spy/partial mock directly.
    $controller->__invoke($request, $moduleName);

    // Assert that the 'authorize' method was called exactly once with the correct argument.
    $controller->shouldHaveReceived('authorize')->once()->with('manage modules');
});
...
test('invoke throws authorization exception if user cannot manage modules', function () {
    ...
    // Ensure protected methods can be mocked if 'authorize' is protected.
    $controller->shouldAllowMockingProtectedMethods();
