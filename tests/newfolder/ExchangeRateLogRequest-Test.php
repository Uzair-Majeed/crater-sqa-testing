...
// Helper for a dynamic partial mock of ExchangeRateLogRequest
function exchangeRateRequestMock(array $methods) {
    return m::mock(ExchangeRateLogRequest::class . '[' . implode(',', $methods) . ']');
}
...
test('getExchangeRateLogPayload returns null when request currency id matches company currency id (same type)', function () {
    ...
    $request = exchangeRateRequestMock(['header', 'validated']);
