 ...
 * Accepts arrays for $resource.
 */
class MockEstimateItemResource extends JsonResource
{
    public function toArray($request)
    {
        // $this->resource is already an array here.
        return array_merge(is_array($this->resource) ? $this->resource : (array) $this->resource, ['transformed_by_mock' => true]);
    }
}

// Helper to create a collection of stdClass from array data
function stdClassCollection($arrayOfArrays): Collection
{
    return collect($arrayOfArrays)->map(function ($arr) {
        return (object) $arr;
    });
}
...
test('toArray correctly transforms a collection with a single item', function () {
    ...
    $collection = stdClassCollection([$itemData]);
...
test('toArray correctly transforms a collection with multiple items', function () {
    ...
    $collection = stdClassCollection([$itemData1, $itemData2]);
...
test('toArray throws a TypeError if a non-Request object is passed as request parameter', function () {
    ...
    expect(fn() => $estimateItemCollection->toArray('this is a string, not a request'))
