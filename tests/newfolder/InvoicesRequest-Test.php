...
use Mockery; // Ensure Mockery is explicitly imported for clarity and proper usage
...
test('rules method returns base validation rules with default unique invoice number', function () use ($companyId, $customerId) {
    ...
    // Use 'overload' mocks for static methods on concrete classes to prevent "class already exists" errors
    ...
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with(Mockery::any())
...
test('rules method makes exchange_rate required when customer currency differs from company currency', function () use ($companyId, $customerId, $companyCurrency, $customerCurrency) {
    ...
    // Fix: Pass initial properties to the Mockery object's constructor
    // This avoids `setAttribute()` errors when setting properties like `$mockCustomer->currency_id`
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $customerCurrency]); // Customer currency is EUR, different from company
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
...
test('rules method keeps exchange_rate nullable when customer currency is same as company currency', function () use ($companyId, $customerId, $companyCurrency) {
    ...
    // Fix: Pass initial properties to the Mockery object's constructor
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $companyCurrency]); // Customer currency is USD (same as company)
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
        ->andReturn($mockCustomer);
...
test('rules method keeps exchange_rate nullable if company currency setting not found', function () use ($companyId, $customerId, $customerCurrency) {
    ...
    // Fix: Pass initial properties to the Mockery object's constructor
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $customerCurrency]);
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
        ->andReturn($mockCustomer);
...
test('rules method modifies invoice_number rule for PUT requests', function () use ($companyId, $invoiceId) {
    ...
    Mockery::mock('overload:' . CompanySetting::class)
        ->shouldReceive('getSetting')
        ->with(Mockery::any(), Mockery::any()) // Expect any call to getSetting
        ->andReturn(null); // Default to null for simplicity in this specific test
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with(Mockery::any())
        ->andReturn(null);
...
test('getInvoicePayload returns correct data with exchange rate applied when currencies differ', function () use (
    ...
    // Fix: Pass initial properties to the Mockery object's constructor
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $customerCurrencyId]); // Customer currency is EUR, different from company
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
        ->andReturn($mockCustomer);
...
test('getInvoicePayload returns correct data with exchange rate of 1 when currencies are same', function () use (
    ...
    // Mock Customer::find static method call
    // Fix: Pass initial properties to the Mockery object's constructor
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $companyCurrencyId]); // Customer currency is USD (same as company)
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
        ->andReturn($mockCustomer);
...
test('getInvoicePayload handles missing company settings with default values', function () use (
    ...
    // Mock Customer::find static method call
    // Fix: Pass initial properties to the Mockery object's constructor
    $mockCustomer = Mockery::mock(Customer::class, ['currency_id' => $customerCurrencyId]);
    Mockery::mock('overload:' . Customer::class)
        ->shouldReceive('find')
        ->with($customerId)
        ->andReturn($mockCustomer);
