...
use Silber\Bouncer\Bouncer; // Import the concrete Bouncer class
use Mockery as m;
...
test('viewAny returns true if user can view any payment', function () {
    // FIX 1: Mock the concrete Bouncer service, not the facade alias,
    // and bind it to the Laravel container.
    $bouncerMock = m::mock(Bouncer::class);
    $bouncerMock->shouldReceive('can')
        ->once()
        ->with('view-payment', Payment::class)
        ->andReturn(true);
    app()->instance('bouncer', $bouncerMock); // Bind the mock to the 'bouncer' service key
...
test('viewAny returns false if user cannot view any payment', function () {
    // FIX 1: Mock the concrete Bouncer service
    $bouncerMock = m::mock(Bouncer::class);
    $bouncerMock->shouldReceive('can')
        ->once()
        ->with('view-payment', Payment::class)
...
test('view returns true if user can view specific payment and belongs to company', function () {
    ...
    $policy = new PaymentPolicy();
    $user = m::mock(User::class);
    // FIX 2: Set properties during mock creation to avoid `setAttribute` errors
    $payment = m::mock(Payment::class, ['company_id' => 1]);
...
test('view returns false if user can view specific payment but does not belong to company', function () {
    ...
    $policy = new PaymentPolicy();
    $user = m::mock(User::class);
    // FIX 2: Set properties during mock creation
    $payment = m::mock(Payment::class, ['company_id' => 1]);
...
afterEach(function () {
    Mockery::close();
    // Clean up the Bouncer mock from the container after each test
