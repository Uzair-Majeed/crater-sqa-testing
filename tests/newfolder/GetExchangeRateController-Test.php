...
test('it returns exchange rate from provider if available and successful', function () {
    ...
    // FIX: Mock property access instead of direct assignment to avoid Mockery\Exception\BadMethodCallException related to setAttribute.
    // Eloquent models handle property access via __get which calls getAttribute.
    $inputCurrency->shouldReceive('getAttribute')->with('id')->andReturn(100);
    $inputCurrency->shouldReceive('getAttribute')->with('code')->andReturn('USD');
    ...
    // FIX: Mock property access instead of direct assignment.
    $baseCurrency->shouldReceive('getAttribute')->with('id')->andReturn(200);
    $baseCurrency->shouldReceive('getAttribute')->with('code')->andReturn('EUR');
