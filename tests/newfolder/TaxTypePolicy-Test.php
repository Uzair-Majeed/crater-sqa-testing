...
use Mockery; // Explicitly import Mockery
use function Pest\Laravel\mock; // This helper is useful for binding to the container, not plain Mockery mocks

beforeEach(function () {
    // Mock BouncerFacade for all tests in this file.
    // We create a Mockery mock instance and bind it to the application container
    // so that any resolution of BouncerFacade (even static calls via the Facade pattern)
    // will hit our mock. We store it in $this->bouncerMock to set expectations directly on it.
    $this->bouncerMock = Mockery::mock(BouncerFacade::class);
    app()->instance(BouncerFacade::class, $this->bouncerMock);
});
...
test('viewAny returns true if user can view any tax types', function () {
    ...
    // Set expectations on the stored BouncerFacade mock instance
...
test('view returns true if user can view specific tax type and has company access', function () {
    $user = mock(User::class);
    // Use makePartial() for Eloquent models when setting properties directly,
    // as it allows the original __set method (which calls setAttribute) to be used.
    $taxType = Mockery::mock(TaxType::class)->makePartial();
    $policy = new TaxTypePolicy();
...
test('view returns false if user cannot view specific tax type', function () {
    ...
    // Ensure hasCompany is not called if Bouncer denies
