...
test('it returns mail configuration with valid from name and address', function () {
    ...
    // The config() helper internally calls app('config')->get($key, $default).
    // When called without a default, Laravel's internal helper passes null as the default.
    Config::shouldReceive('get')
        ->once()
        ->with('mail.from.name', null) // Expect key and null default argument
        ->andReturn($expectedFromName);

    Config::shouldReceive('get')
        ->once()
        ->with('mail.from.address', null) // Expect key and null default argument
...
test('it returns mail configuration with null values if config keys are not set', function () {
    ...
    // Ensure the mock also expects the null default argument from the config() helper.
    Config::shouldReceive('get')
        ->once()
