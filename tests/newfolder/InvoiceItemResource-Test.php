...
test('toArray transforms all direct properties and includes existing relations', function () {
    ...
    // For relations that should be included, set the property to a collection.
    // JsonResource's 'whenLoaded' method checks if the property exists on the model.
    $mockModel->taxes = $mockTaxCollection;
    $mockModel->fields = $mockFieldCollection;

    // Remove shouldReceive('exists') mocks as they are not typically used by JsonResource
    // for loaded relations. whenLoaded() checks if the relation property is set.
...
test('toArray does not include relations if they do not exist', function () {
    ...
    // To ensure relations are NOT included by JsonResource (e.g., via whenLoaded),
    // set the relation properties to null or don't set them at all.
    $mockModel->taxes = null;
    $mockModel->fields = null;

    // Remove shouldReceive('exists') mocks as they are not relevant if whenLoaded is used.
...
test('toArray handles null or missing properties gracefully', function () {
    ...
    // To ensure relations are NOT included, set them to null.
    $mockModel->taxes = null;
    $mockModel->fields = null;

    // Remove shouldReceive('exists') mocks.
...
test('toArray handles a mix of existing and non-existing relations', function () {
    ...
    $mockModel->taxes = $mockTaxCollection;
    // Remove shouldReceive for taxes, as it's correctly set.

    // Fields should NOT be included
    $mockModel->fields = null; // Set to null to ensure whenLoaded returns false
    // Remove shouldReceive for fields.
...
test('toArray includes empty collection for relation if it exists but the collection is empty', function () {
    ...
    // To ensure relations ARE included as empty collections,
    // set the relation properties to empty collections.
    // whenLoaded() will return true because the property exists.
    $mockModel->taxes = collect([]);
    $mockModel->fields = collect([]);

    // Remove shouldReceive('exists') mocks as they are not relevant for this scenario.
