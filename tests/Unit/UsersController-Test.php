...
use Mockery; // Add this line to import Mockery
...
test('index method uses custom limit if provided in request', function () {
    ...
    // Correctly mock the input method for the limit, assuming the controller uses ->input() or ->get()
    $request->shouldReceive('input')->with('limit')->andReturn(25);
    $request->shouldReceive('user')->andReturn($currentUser);
...
test('delete method authorizes and deletes specified users when users are provided in request', function () {
    ...
    // Or, if using validated data, mock that: $request->shouldReceive('validated')->andReturn(['users' => $userIds]);
    $request->users = $userIds; // Assuming direct property access from the FormRequest
...
test('delete method authorizes but does not delete users when no users are provided in request', function () {
    ...
    $request->users = null; // Assuming direct property access from the FormRequest
