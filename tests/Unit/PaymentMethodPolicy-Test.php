...
beforeEach(function () {
    ...
    test('view allows access if user can view payment and belongs to the payment method company', function () {
        ...
        // Fix: Mockery throws BadMethodCallException when setting properties directly on a strict mock of an Eloquent Model.
        // Policies typically read properties using __get, which on a model, internally calls getAttribute.
        // We mock getAttribute to return the desired companyId.
        $paymentMethod->shouldReceive('getAttribute')
            ->once()
            ->with('company_id')
            ->andReturn($companyId);
    ...
    test('view denies access if user cannot view payment (short circuit)', function () {
        ...
        $user = Mockery::mock(User::class);
        $paymentMethod = Mockery::mock(PaymentMethod::class);
        // Fix: Even though hasCompany isn't called, the policy might still try to access company_id before the `&&` check.
        // Ensure the attribute can be read if the policy attempts it.
        $paymentMethod->shouldReceive('getAttribute')
            ->zeroOrMoreTimes() // Policy might access it, but the `&&` short-circuit means hasCompany won't be called.
            ->with('company_id')
            ->andReturn($companyId);
    ...
    test('update denies access if user cannot view payment (short circuit)', function () {
        ...
        $user = Mockery::mock(User::class);
        $paymentMethod = Mockery::mock(PaymentMethod::class);
        // Fix: Ensure the attribute can be read if the policy attempts it.
        $paymentMethod->shouldReceive('getAttribute')
            ->zeroOrMoreTimes()
            ->with('company_id')
            ->andReturn($companyId);
