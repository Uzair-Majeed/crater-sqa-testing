...
test('it transforms basic properties and omits company when the relationship does not exist', function () {
    ...
    // Ensure there's no 'company' property on the model (simulate absence)
    unset($mockModel->company);
    ...
    // The resource may include the 'company' key but with a null value,
    // so assert it is either not present, or present and null:
    ...
    expect(
        !array_key_exists('company', $result)
        || $result['company'] === null
    )->toBeTrue();
});
