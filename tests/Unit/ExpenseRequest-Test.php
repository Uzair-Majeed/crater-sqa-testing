...
/**
 * Create a Mockery alias for CompanySetting before each test.
 * This allows non-static mocking of static methods.
 */
beforeEach(function () {
    // Register CompanySetting as a Mockery alias so shouldReceive works.
    if (!class_exists('CompanySettingAlias')) {
        class_alias(CompanySetting::class, 'CompanySettingAlias');
    }
    \Mockery::mock('alias:CompanySettingAlias');
});

/**
 * Helper to create ExpenseRequest and inject mocked Request data.
 */
function createExpenseRequestWithData(array $data = [], array $headers = [])
{
    ...
    // Inject request using reflection, if needed
    if (property_exists($expenseRequest, 'request')) {
        $reflection = new ReflectionClass($expenseRequest);
        $requestProperty = $reflection->getProperty('request');
        $requestProperty->setAccessible(true);
        $requestProperty->setValue($expenseRequest, $mockRequest);
    }
    // Set Laravel request property
    if (method_exists($expenseRequest, 'setLaravelRequest')) {
        $expenseRequest->setLaravelRequest($mockRequest);
    } else {
        $expenseRequest->initialize(
...
test('rules method returns base rules when company currency is not set', function () {
    CompanySettingAlias::shouldReceive('getSetting')
        ->once()
        ->with('currency', '1')
        ->andReturn(null);
...
test('getExpensePayload handles cases where currency_id or amount might be missing from request', function () {
    ...
    expect($payload['exchange_rate'])->toBe(1);
    expect($payload['currency_id'])->toBeNull();
...
test('getExpensePayload handles zero amount correctly', function () {
    ...
    expect($payload['base_amount'])->toBe(0.00);
    expect($payload['exchange_rate'])->toBe(1);
});
