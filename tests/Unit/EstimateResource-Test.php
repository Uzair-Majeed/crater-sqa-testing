...
function mockJsonResourceConstructorInstance($resourceData) {
    // Create a dynamic proxy subclass extending JsonResource for correct behavior.
    return new class($resourceData) extends JsonResource {
...
test('toArray includes all direct properties and omits relationships when they do not exist', function () {
    ...
    expect($result['estimate_pdf_url'])->toBe('http://example.com/estimate.pdf');
...
test('toArray includes all relationships when they exist and are loaded', function () {
    ...
    m::mock('alias:'.EstimateItemResource::class)
        ...
        ->once()
        ->with(m::on(fn($arg) => $arg === $mockItemsData))
    ...
    m::mock('overload:'.CustomerResource::class)
        ...
        ->andReturnUsing(function($resource) {
            $instance = mockJsonResourceConstructorInstance($resource);
            // workaround for Pest/Mockery 'overload': the resource is a real instance, so `new CustomerResource($model)` will trigger this
            return $instance;
        });
    ...
    expect($result['customer'])->toBeInstanceOf(JsonResource::class);
    expect($result['customer']->resource)->toBe($mockCustomerData);

    expect($result['creator'])->toBeInstanceOf(JsonResource::class);
    expect($result['creator']->resource)->toBe($mockCreatorData);

    expect($result['company'])->toBeInstanceOf(JsonResource::class);
    expect($result['company']->resource)->toBe($mockCompanyData);

    expect($result['currency'])->toBeInstanceOf(JsonResource::class);
...
test('toArray handles null and empty values for all properties and omitted relationships', function () {
    ...
    expect($result['estimate_pdf_url'])->toBeNull();
