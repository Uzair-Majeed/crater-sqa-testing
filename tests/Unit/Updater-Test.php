...
use Mockery; // <-- FIX: Added missing Mockery import
use ZipArchive;
...
function phpversion($extension = null)
{
    ...
    if ($extension === 'json') { return '1.9.0'; } // Default to existing for common extensions
    return false;
}
...
function file_put_contents($filename, $data, $flags = 0) // Added $flags for complete signature
{
...
test('checkForUpdate processes successful response with extensions and php version check', function () {
    ...
    $mockedResponseData = (object) [ // Cast to object as the production code likely accesses properties dynamically
        'success' => true,
        'version' => (object) [
            'version' => '2.0.0',
    ...
    Mockery::mock('alias:'.Updater::class)
        ...
        ->shouldReceive('getRemote')
        ->once()
        ->andReturn(new Response(200, [], json_encode($mockedResponseData))); // Encode the object for response body
...
test('checkForUpdate handles response without extensions property', function () {
    ...
    // It should add the php version check
    expect($result->version)->toHaveProperty('extensions');
    expect($result->version->extensions)->toBeArray()->toHaveCount(1);
    expect(array_key_exists('php(8.0)', $result->version->extensions))->toBeTrue();
    expect($result->version->extensions['php(8.0)'])->toBeTrue();
});
...
test('download constructs correct URL for development environment', function () {
    ...
    // The beforeEach mock handles this, but explicitly mocking for specific expectations is better.
...
test('download handles RequestException from getRemote', function () {
    Mockery::mock('alias:'.Updater::class)
        ...
        ->shouldReceive('getRemote')
        ->once()
        ->andThrow(new RequestException('Download error', new \GuzzleHttp\Psr7\Request('GET', 'test'))); // Use andThrow for exceptions
...
test('unzip creates extract directory if it does not exist', function () {
    ...
    $zipArchiveMock->shouldReceive('open')->with($zipFilePath, \ZipArchive::CREATE)->once()->andReturn(true); // Added ZipArchive::CREATE flag
...
test('unzip does not create extract directory if it already exists', function () {
    ...
    $zipArchiveMock = Mockery::mock('overload:' . ZipArchive::class);
    $zipArchiveMock->shouldReceive('open')->with($zipFilePath, \ZipArchive::CREATE)->once()->andReturn(true);
    $zipArchiveMock->shouldReceive('extractTo')->once()->andReturn(true);
    $zipArchiveMock->shouldReceive('close')->once()->andReturn(true);
...
test('unzip handles ZipArchive open failure gracefully', function () {
    ...
    $zipArchiveMock = Mockery::mock('overload:' . ZipArchive::class);
    $zipArchiveMock->shouldReceive('open')->with($zipFilePath, \ZipArchive::CREATE)->once()->andReturn(false); // Simulate open failure
    ...
    // The method throws an exception if open fails, so we need to assert that.
    // The previous test logic for `unzip handles ZipArchive open failure gracefully` was wrong as it expected
    // it to return a string, but the actual implementation of Updater::unzip
    // throws an exception for open failure.
    expect(fn() => Updater::unzip($zipFilePath))
        ->toThrow(\Exception::class, 'Could not open zip file');

    // Ensure delete is still called
    File::shouldHaveReceived('delete')->once()->with($zipFilePath);
});
...
test('migrateUpdate calls Artisan migrate command', function () {
    Artisan::shouldReceive('call')
        ->once()
        ->with('migrate', ['--force' => true]); // Added named argument for force flag to match Artisan::call signature better
